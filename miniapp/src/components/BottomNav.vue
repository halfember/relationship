...
const requireLogin = () => {
  uni.showModal({
    title: '需要登录',
    content: '登录后可使用关系管理、日历和个人资料功能。',
    confirmText: '去登录',
    cancelText: '暂不登录',
    success: ({ confirm }) => { if (confirm) uni.navigateTo({ url: '/pages/login/login' }); },
  });
};
const go = item => {
  if (navigating.value) return;
  if (item.key === 'create') {
    if (!uni.getStorageSync('userInfo')?.accessToken) return requireLogin();
    return openContactCreationMenu();
  }
  if (item.key !== 'home' && !uni.getStorageSync('userInfo')?.accessToken) return requireLogin();
...
.nav-item:nth-child(3) .nav-glyph { width:72rpx;height:72rpx;border-radius:50%;background:#315c4d;color:#fff;font-size:40rpx; }
