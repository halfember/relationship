<template>
  <view class="page-shell space-list-page">
    <view class="page-head">
      <view><text class="page-title">共同空间</text><text class="page-sub">和重要的人一起记录</text></view>
    ...
    <view v-else-if="visible.length" class="space-list">
      <view v-for="item in visible" :key="item.id" class="space-row" @tap="open(item)">
        ...
        <view class="space-copy">
          ...
          <text class="space-meta">{{ item.type === 'FAMILY' ? '家庭空间' : '双人空间' }} · {{ item.members?.length || 0 }} 位成员</text>
    ...
    <view v-else class="empty surface">
      ...
      <text class="empty-title">还没有共同空间</text>
      <text class="empty-desc">邀请一个人一起记录，或为家人创建家庭空间。</text>
...
const tabs = [{ key: 'all', label: '全部' }, { key: 'PAIR', label: '双人' }, { key: 'FAMILY', label: '家庭' }];
const visible = computed(() => filter.value === 'all' ? spaces.value : spaces.value.filter(item => item.type === filter.value));
...
const avatarText = item => item.type === 'FAMILY' ? (item.name || '家').slice(0, 1) : (item.members?.[1]?.displayName || item.name || '双').slice(0, 1);
const open = item => {
  ...
  uni.showActionSheet({ itemList: ['复制邀请码', '撤回邀请'], success: async ({ tapIndex }) => {
    if (tapIndex === 0) uni.setClipboardData({ data: invite.token });
    if (tapIndex === 1) {
