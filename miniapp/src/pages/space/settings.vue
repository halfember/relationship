...
const createInvite=async()=>{const invite=await spaceApi.createInvite(spaceId.value);uni.showModal({title:'家庭邀请码',content:`${invite.token}\n7天内有效，可在共同空间中输入`,confirmText:'复制',success:r=>r.confirm&&uni.setClipboardData({data:invite.token})});};
const leave=()=>uni.showModal({title:'退出空间',content:'退出后将无法继续查看共同内容，私人记录不受影响。',confirmText:'确认退出',confirmColor:'#d85858',success:async r=>{if(r.confirm){await spaceApi.leave(spaceId.value);uni.reLaunch({url:'/pages/space/list'});}}});
const dissolve=()=>uni.showModal({title:'解散空间',content:'所有成员将无法继续访问，共同内容会保留为归档数据。',confirmText:'确认解散',confirmColor:'#d85858',success:async r=>{if(r.confirm){await spaceApi.dissolve(spaceId.value);uni.reLaunch({url:'/pages/space/list'});}}});
</script>

<style scoped>
.settings-page{min-height:100vh;padding:24rpx 28rpx;background:#f7f7f5}.form-card{padding:22rpx;background:#fff;border:1rpx solid #dedfd9;border-radius:12rpx}.label{display:block;margin-bottom:14rpx;font-size:32rpx;font-weight:600}.name-row{display:flex;gap:12rpx}.name-row input{flex:1;height:88rpx;padding:0 16rpx;background:#f7f7f5;border-radius:11rpx;font-size:32rpx}.name-row button{width:120rpx;height:88rpx;line-height:88rpx;margin:0;padding:0;border-radius:11rpx;background:#315c4d;color:#fff;font-size:28rpx}.group-title{display:block;margin:30rpx 0 10rpx;color:#8c8f9c;font-size:28rpx}.list{overflow:hidden;background:#fff;border:1rpx solid #dedfd9;border-radius:12rpx}.row{height:88rpx;padding:0 22rpx;display:flex;align-items:center;justify-content:space-between;border-bottom:1rpx solid #e2e5e0;font-size:32rpx}.row:last-child{border:0}.row text:last-child{color:#8b918d;font-size:28rpx}.row.danger,.row.danger text:last-child{color:#d85858}.privacy{padding:22rpx;border-radius:10rpx;background:#e4ece7;color:#555e58;font-size:28rpx;line-height:1.7}.loading{padding:160rpx;text-align:center;color:#8b918d}
</style>
