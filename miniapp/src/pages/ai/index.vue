  ...
  <view class="page-shell ai-home">
    <view class="assistant-hero">
      <view class="robot">
        ...
        <view class="robot-head"><view class="robot-screen"><text>•</text><text>•</text><view class="smile"></view></view></view>
        <view class="robot-body">AI</view>
      </view>
      <text class="hero-title">Hi，我是你的关系助手</text>
      <text class="hero-subtitle">从祝福、纪念日文案和送礼建议开始</text>
    </view>
    ...
    <view v-if="recordsLoading" class="recent-state"><text>正在加载最近记录…</text></view>
    <view v-else-if="recordsError" class="recent-state recent-error"><text>{{ recordsError }}</text><button @tap="loadRecords">重新加载</button></view>
    <view v-else-if="records.length" class="recent-list">
    ...
    <view v-else class="recent-state"><text class="recent-name">开始第一次 AI 创作</text><text class="recent-desc">选择上方工具，生成贴心内容</text></view>
  </view>
</template>
...
const tools = [
  { title: '祝福语生成', desc: '为重要日子生成祝福', icon: '✿', bg: '#eeeaff', color: '#7560e7', url: '/pages/ai/blessing' },
  { title: '纪念日文案', desc: '生成纪念日文案', icon: '♥', bg: '#ffe6ef', color: '#e76592', url: '/pages/ai/memory-text' },
  { title: '送礼建议', desc: '智能推荐合适礼物', icon: '♙', bg: '#fff0dd', color: '#ec9b2b', url: '/pages/ai/gift' },
];
...
const loadRecords = async () => {
  ...
  recordsError.value = '';
  try {
    const data = await aiApi.records(store.userId, 1, 5);
    records.value = data?.list || [];
  } catch (error) {
    records.value = [];
    recordsError.value = error?.message || '最近记录暂时不可用，请稍后重试';
  } finally {
